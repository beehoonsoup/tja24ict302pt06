...
exports.getBiographyPage = async (req, res) => {
    try {
        ...
        res.render(`biography`, { user, feedback, userId, bio, skills, project });
    } catch (error) {
        console.error(error);
...
exports.viewBiography = async (req, res) => {
    try {
        ...
        const [skills] = await db.query('SELECT se.UserID, se.SkillID, s.SkillName, COUNT(*) FROM SelfEvaluation se INNER JOIN Skill s ON s.SkillID = se.SkillID WHERE se.UserID = ? GROUP BY se.UserID, se.SkillID, s.SkillName ORDER BY COUNT(*) DESC', [reviewerID])

        const [project] = await db.query('SELECT p.ProjectID, p.ProjectName, t.TeamRole FROM Project p INNER JOIN Team t ON t.ProjectID = p.ProjectID WHERE t.TeamStatus = "Verified" AND t.UserID = ?', [reviewerID])

        //console logs
        //console.log('user', user);

        //res.render('biography', { user, feed, projectFeed, selfEvaluation });
        res.render('biography', { user, reviewerID, feedback, userId, bio, skills, project });
    } catch (error) {
        console.error(error);
        res.status(500).send('An error occurred');
    }
